...
std::optional<oops::classes::clazz> clazz::superclass() const {
  void *superclass =
      this->class_data.read<void *>(CLASS_HEADER_BYTE_OFFSET::SUPERCLASS);
  if (superclass) {
...
std::uint32_t clazz::object_size() const {
  return this->class_data.read<std::uint32_t>(
      CLASS_HEADER_BYTE_OFFSET::OBJECT_SIZE);
}

std::uint16_t clazz::instance_pointer_count() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::IPOINTER);
}
std::uint16_t clazz::instance_double_count() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::IDOUBLE);
}
...
std::uint16_t clazz::static_pointer_count() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::SPOINTER);
}
std::uint16_t clazz::static_double_count() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::SDOUBLE);
}
...
std::uint32_t clazz::virtual_method_count() const {
  return this->class_data.read<std::uint32_t>(
      CLASS_HEADER_BYTE_OFFSET::VIRTUAL_METHOD_COUNT);
}

std::uint32_t clazz::total_reflection_count() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::REFLECTION_STRING_COUNT);
}

std::uint32_t clazz::total_bytecode_size() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::BYTECODE_SIZE);
}

std::uint32_t clazz::total_string_pool_size() const {
  return this->class_data.read<std::uint16_t>(
      CLASS_HEADER_BYTE_OFFSET::STRING_POOL_COUNT);
}
